...
import { commerceIntegrationPlan } from './commerce-integration-plan.js';

export function registerIntegrationPlanPrompt(server: McpServer, provider: DocsProvider): void {
  server.prompt(
    'integration-action-plan',
    'PG 결제 또는 Commerce 쇼핑몰 연동 액션 플랜을 생성합니다. 일반결제/정기결제는 PG, 주문서/구독은 Commerce v1 계약과 자격증명 흐름을 사용합니다.',
    {
      ...
      framework: z.enum(['react', 'vue', 'nuxt', 'nextjs', 'svelte', 'angular']).optional().describe('Commerce 웹 프레임워크. 생략하면 기존 프로젝트에서 확인하며 네이티브/서버 언어 선택은 유지합니다.'),
    },
    async ({ type, platform, server_language, framework }) => {
      ...
      if (type === '주문서' || type === '구독') {
        return {
          messages: [{
            ...
            content: {
              type: 'text' as const,
              text: commerceIntegrationPlan({ type, platform, serverLanguage: lang, framework }),
            },
          }],
        };
      }
...
- [ ] done 이벤트에서 receipt_id 수신 → 서버로 전달 (billing_key는 프론트로 오지 않음)` : ''}
...
- [ ] 해지 — 대기 예약 취소(cancelSubscribeReserve) 후 destroyBillingKey` : ''}
...
- [ ] 결제 취소 시나리오
- [ ] 웹훅 수신 확인
${type === '정기결제' ? '- [ ] 빌링키 발급 → 자동결제 → 해지 전체 흐름' : ''}
