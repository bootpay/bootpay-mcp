...
> ⚠️ **이 버전 정보를 반드시 사용하세요.** 3.x, 4.x 등은 v1 구버전(deprecated)이며 동작하지 않습니다. 반드시 v2 (5.x 이상)를 사용하세요.
...
## 1. 계정 & 프로젝트 준비
- [ ] [Bootpay 관리자](https://admin.bootpay.ai) 회원가입 완료
- [ ] 사업자 정보 등록 완료
- [ ] [프로젝트 생성](https://admin.bootpay.ai/project/new) 완료
- [ ] 사용할 PG사 활성화 ([결제 설정](https://admin.bootpay.ai/payment/setting))
- [ ] Sandbox / Production 환경 결정

> ⚠️ **Application ID 미설정은 가장 흔한 연동 실패 원인입니다.**
> 반드시 [관리자](https://admin.bootpay.ai/setting/developer?tab=api-key&cursor=payment)에서 발급받은 실제 ID를 사용하세요.
...
[관리자 → 개발자 설정 → API 연동키 (결제)](https://admin.bootpay.ai/setting/developer?tab=api-key&cursor=payment)
...
[관리자 → 개발자 설정 → API 연동키 (커머스)](https://admin.bootpay.ai/setting/developer?tab=api-key&cursor=internal)
...
- [ ] **서버 SDK 사용**: 자동결제는 반드시 서버에서 \`requestSubscribeCardPayment()\`로 호출합니다
- [ ] **토큰 발급**: 서버 SDK에서 \`getAccessToken()\`을 먼저 호출했나요?
...
| 자동결제 | **서버** | \`requestSubscribeCardPayment()\` | billing_key로 결제 (사용자 개입 없음) |
...
4. **billing_key를 프론트엔드에 저장** → 서버 DB에 암호화하여 저장하세요
5. **토큰 미발급** → \`getAccessToken()\`을 먼저 호출해야 합니다
...
| 에러 코드 | 원인 | 해결 |
|-----------|------|------|
| **-100** (APPLICATION_ID_INVALID) | 잘못된 Application ID | 관리자에서 올바른 ID 확인 |
| **-200** (TOKEN_EXPIRED) | 토큰 만료 | \`getAccessToken()\` 재호출 |
...
> 자세한 내용: \`get_doc("integration/errors")\``,
...
- [ ] **Application ID**: 올바른 ID를 사용하고 있나요?
  ...
  server.prompt(
    ...
    async ({ type, platform, server_language }) => {
      ...
      const typeConfig: Record<string, { apiDomain: string; docPaths: string[]; description: string }> = {
        ...
        '정기결제': {
          apiDomain: 'api.bootpay.co.kr',
          docPaths: ['billing/intro', 'billing/request', 'billing/reserve'],
...
- [ ] [Bootpay 관리자](https://admin.bootpay.ai) 회원가입
- [ ] 사업자 정보 등록
- [ ] 프로젝트 생성
