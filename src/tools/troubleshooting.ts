...
export function registerTroubleshootingTool(server: McpServer): void {
  server.tool(
    ...
    {
      topic: z
        .enum(['onboarding', 'sandbox', 'webhook', 'billing', 'subscription', 'error', 'cancel', 'cors', 'csp', 'local-network', 'open-type', 'mobile', 'widget', 'unified', 'certification'])
        .describe('문제 유형: onboarding(가입·PG 신청·가맹 심사·결제수단 활성화 — 「결제창이 안 뜬다」·「심사가 안 끝난다」의 대부분이 여기), sandbox(테스트 환경), webhook(웹훅 미수신), billing(빌링키 발급·자동결제), subscription(구독 운영 — 이중 결제·배치 미실행·해지 후 청구·무료체험·가격 변경), error(에러코드), cancel(취소·부분취소·환불 상태값), cors(허용 도메인 등록), csp(Content-Security-Policy 로 결제창 차단 — 「Refused to frame」·frame-src), local-network(크롬 로컬 네트워크 정책으로 ISP·페이북 인증창 차단 — 「100만원 넘으면 인증창이 안 뜬다」·loopback), open-type(iframe·popup·redirect 선택과 기기별 분기, 결과 이벤트 처리), mobile(앱), widget(결제위젯), unified(통합결제창), certification(본인인증)'),
    },
    TOOL_READ_LOCAL,
    async ({ topic }) => {
...
- [ ] **실패 코드**: \`SUBSCRIBE_PUBLISH_FAILED\` 면 원인은 함께 온 \`pg_error_code\` 에 있습니다 (나이스 \`F115\`·\`F117\` 빌링 권한 없음, KCP \`5209\` GROUP_ID) → topic="error"

> **계좌 자동결제(계좌자동이체)도 됩니다 — 나이스페이먼츠 경유.** 계좌 기반 빌링키를 발급해 결제하며, 발급에 \`bank_code\`·\`bank_account\`·\`username\`·\`identity_no\`·\`phone\` 이 필요합니다(빠지면 \`SUBSCRIBE_AT_*\`). ⛔ "카드 빌링키만 지원한다"고 안내하지 마세요.
...
## \`error_code\` 와 \`pg_error_code\` 가 같이 올 때

\`error_code\` 는 부트페이가 **어느 단계에서** 실패했는지, \`pg_error_code\`·\`message\` 는 PG가 **왜** 거절했는지다.
아래 두 코드는 원인이 \`pg_error_code\` 에 있으니 그 값으로 가른다. 부트페이 코드만 보고 연동 코드를 고치지 마세요.

| error_code | pg_error_code | 메시지 | 원인 · 처방 |
|---|---|---|---|
| \`RC_REQUEST_FAILED\` | 카카오페이 \`-799\` | approve_url(https://gw.bootpay.co.kr/return/kakao…) does not match registered website domain | 카카오페이가 구버전(카카오 개발자 \`admin_key\`) 방식이라 카카오 쪽에 부트페이 도메인이 없음. 관리자 카카오페이 설정을 **신버전 모드로 바꾸고 \`admin_key\` 를 비운다**. v1 시절 같은 메시지는 \`-15100\` |
| \`RC_REQUEST_FAILED\` | 이니시스 \`V101\` | 선택한 결제수단으로는 결제가 불가능합니다 | 부트페이 관리자에서 PG·결제수단 활성화 → 그래도 나면 이니시스 관리자에서 그 수단이 켜져 있는지 확인 |
| \`RC_REQUEST_FAILED\` | 이니시스 \`V023\` | 계약되지 않은 인증방법입니다 | 이니시스 계약 범위 밖의 인증방식. 이니시스 계약 확인 |
| \`SUBSCRIBE_PUBLISH_FAILED\` | 나이스 \`F115\` | 빌키 발급 불가 가맹점입니다.(중지) | MID에 카드 빌링 계약이 없거나 나이스 전산 미반영, 또는 장기 미사용으로 MID 정지. 나이스에 빌링 사용 가능 여부 확인 |
| \`SUBSCRIBE_PUBLISH_FAILED\` | 나이스 \`F117\` | 빌링 미사용 가맹점입니다 | 같은 계열(빌링 권한 없음) |
| \`SUBSCRIBE_PUBLISH_FAILED\` | KCP \`5209\` | [배치빌] 존재하지않는(사용불가) GROUP_ID | 위 PG 원문 에러 표 참고 |
| \`RC_CONFIRM_FAILED\` | 나이스 \`1512\` | 회원사 서비스 개시 안됨 | MID가 아직 서비스 개시(최종 승인) 전이거나 설정 미완료. 실결제 전환 직후 흔함. 나이스에 개시 여부 확인. 이니시스 \`MX2003\` "상점 서비스가 개시 상태가 아닙니다" 도 같은 뜻 |

## \`Translation missing: ko.error.code.1216\` — \`APP_CLIENT_KEY_NOT_FOUND\`

1216 은 \`APP_CLIENT_KEY_NOT_FOUND\` 이고 메시지 번역이 빠진 채 번호가 노출된 것이다. 결제 요청에 실린 client key 가
비어 있거나, 형식이 틀리거나, 존재하지 않을 때 난다.

- **가장 흔한 원인은 client_key 빈 값 전달** — 환경변수가 빌드·런타임에 주입되지 않은 경우. 일부 요청에서만 나면 그 요청에 실린 값부터 로그로 확인
- Secret Key 짝 불일치는 \`APP_SK_NOT_MATCHED\`, 레거시 Application ID 오류는 \`APP_KEY_NOT_FOUND\`

> ⛔ 사용자 데이터의 특수문자·브라우저 환경 탓으로 먼저 돌리지 마세요. 실제 상담(2026-09-07)에서 원인은 client_key 빈 값이었습니다.

## \`TOKEN_KEY_INVALID\` — "ACCESS TOKEN 값을 확인해주세요"

1. 토큰은 받았는데 이후 요청의 \`Authorization\` 헤더에 싣지 않음
2. 만료(30분) 뒤 재사용
3. **토큰을 발급한 프로젝트와 조회·취소하려는 결제건의 프로젝트가 다름** — 다른 프로젝트 결제는 조회되지 않는다

서버 SDK 를 쓰면 토큰 처리를 대신 한다. 결제 취소를 앱에서 REST 로 직접 부르지 말고 서버에서 처리하세요.
...
그건 CSP 로 풀리지 않습니다. 이 경우 \`open_type: 'redirect'\` 로 전환하세요 → topic="open-type"`,
        ...
        'local-network': `# 100만원이 넘으면 인증창이 안 뜬다 — 크롬 로컬 네트워크 정책

## 지문 — 이 세 개가 같이 나오면 확정이다

1. **100만원 이하는 정상 결제된다.** 100만원을 넘을 때만 인증창이 뜨지 않는다
2. KB국민 · BC · 카카오뱅크 카드(**ISP · 페이북**)에서 난다
3. 콘솔에 **Permission was denied for this request to access the loopback address space** 가 찍힌다

100만원 초과는 카드사가 인증서 단계를 추가로 요구하고, 그 단계가 구매자 PC 의 카드사 보안 프로그램을
\`https://127.0.0.1:<포트>\` 로 호출한다. 100만원 이하는 그 호출 자체가 없어서 멀쩡한 것이다.

크롬 142(2025-10)부터 **로컬 네트워크 액세스(LNA)** 정책이 적용돼 이 호출에 권한이 필요해졌다.
결제창을 iframe 으로 열면 이 권한이 다른 도메인 iframe 으로 내려가지 않아(기본 allowlist \`self\`)
**권한 팝업조차 뜨지 않고 조용히 거부**된다.

## 🔴 보안 헤더로 해결되지 않는다 — 여기서 시간을 가장 많이 버린다

"Vercel 이 iframe 정책을 조여서 그렇다" 는 오진이 실제로 나왔다. **아니다.**

| 상황 | 결과 |
|------|------|
| 최상위 페이지 · 팝업 | 허용 (헤더와 무관) |
| 다른 도메인 iframe | 거부 |
| 다른 도메인 iframe + \`Permissions-Policy: loopback-network=*\` 추가 | **여전히 거부** |
| 보안 헤더가 아예 없는 사이트의 iframe | **여전히 거부** |

- \`Permissions-Policy\` 를 추가해도 iframe 안은 그대로 거부된다. iframe 태그 쪽 위임이 함께 필요한데
  그건 결제 SDK 영역이라 가맹점이 손댈 수 없다
- **CSP 와 무관하다.** CSP 는 다른 도메인 iframe 안으로 상속되지 않는다
- 보안 헤더를 한 번도 설정한 적 없는 사이트에서도 똑같이 거부된다 — 브라우저 기본 동작이다
- 오히려 \`loopback-network=(self)\` 처럼 잘못 적으면 없던 제약이 새로 생긴다

**가맹점에게 vercel.json · next.config.js 를 고치라고 안내하지 마세요.** 해결되지 않는다.

## 확인 방법

개발자도구 Console 의 프레임 선택을 **결제창 프레임**으로 바꾸고 실행한다.

\`\`\`javascript
document.featurePolicy.allowsFeature('loopback-network')   // false 면 이 건
\`\`\`

## 해결 — 창 모드를 바꾼다

결제창이 **최상위 창**으로 뜨면 권한이 정상 적용되고 허용 팝업도 정상적으로 나타난다.
...
## ⚠️ 전환하면 결과 수신 방식이 바뀐다 — 같이 안내해야 한다

옵션만 바꾸고 결과 처리를 그대로 두면 **결제는 되는데 주문이 확정되지 않는다.**

| open_type | 결과 수신 | 추가로 해야 할 것 |
|-----------|----------|------------------|
| \`popup\` | \`confirm\` 콜백 (iframe 과 동일) | 거의 없음. 구매자 클릭 1회가 추가된다 |
| \`redirect\` | **\`redirect_url\` 서버 라우트** | 서버 라우트 신설 필수 |

\`redirect\` 로 바꿀 때 반드시 짚어야 할 것:

1. 주문(order_id·금액)을 **결제 요청 전에** 서버 DB 에 저장한다 — 페이지를 떠나면 화면 상태가 사라진다
2. \`redirect_url\` 은 서버가 처리하는 주소여야 한다 (Next.js Route Handler, 클라이언트 컴포넌트 불가)
3. 쿼리로 오는 값은 \`event\` · \`receipt_id\` · \`order_id\` · \`status\` · \`message\` 요약본이다 — \`receipt_id\` 로 재조회
4. \`event=confirm\` 은 인증 완료지 결제 완료가 아니다. 서버에서 금액 대조 후 승인 API 호출
5. 프론트 \`try/catch\` 는 이동 **전** 취소·오류만 잡는다
6. 세션 쿠키가 \`SameSite=Strict\` 면 복귀 시 로그아웃처럼 보인다 → \`Lax\`
7. 새로고침·뒤로가기로 라우트가 다시 실행되므로 같은 \`receipt_id\` 두 번 승인을 막는다
8. \`redirect_url\` 을 빠뜨리면 \`RC_REDIRECT_URL_INVALID\`

PC·모바일을 다르게 쓰면 **양쪽 결과 처리를 모두 구현**해야 한다. 한쪽만 만들면 그 경로 구매자는
결제되고도 주문이 확정되지 않는다. 웹훅을 함께 켜 두면 안전하다.

## 팝업 모드 주의

버튼 클릭과 \`requestPayment()\` 사이에 \`await\`(주문 생성 API 호출 등)를 두면 브라우저가 팝업을 차단한다.
주문 생성은 결제 버튼을 누르기 **전에** 끝내 두게 안내한다.

## 이미 거부한 구매자 안내

주소창 자물쇠 → 사이트 설정 →
크롬 145 이상은 **「기기의 앱」**, 크롬 142~144 는 **「로컬 네트워크 액세스」** 를 **허용** 으로 변경 → 새로고침.
급하면 Microsoft Edge 를 안내할 수 있다.

문서: https://developers.bootpay.ai/payment-window/local-network
`,
...
| \`AUTH_CONFIRM_READY_FAILED\` + KCP \`CS01\` "인증정보가 변조 되었습니다" | 관리자에 넣은 \`web_site_id\` 문제 | PG/결제 설정 → KCP → 본인인증에서 **\`web_site_id\` 를 빈 값으로 저장** 후 재요청. 선택값이라 대부분 비워 두는 게 맞다 |
| KCP \`CS13\` "인증되지 않은 도메인" | KCP 인증결과 URL 에 부트페이 도메인 미등록 | KCP 파트너 → 부가서비스 → 휴대폰본인인증 인증결과 URL 에 \`api\`·\`gw\`·\`admin.bootpay.co.kr\` 등록. 등록 뒤 CS01 이 나면 위 처방 |
| \`AUTH_CONFIRM_READY_FAILED\` + 다날 \`3066\`·\`9017\` | 입력 정보 불일치 · 가입 통신사가 지원하지 않는 기능 | 함께 온 메시지가 원인. 사용자 입력·통신사 확인 |
| \`AUTH_REQUEST_FAILED\` + 다날 \`9014\` "업체정보에 이상이 있습니다" | 다날이 업체 정보(CPID)를 거절 | ① **이용금액 미납**(잘 되다 갑자기면 1순위 — 다날 파트너센터 결제하기) ② CPID·CPPWD 불일치(대소문자) ③ 다날 계정 정지 ④ REST 미허용 CPID(소켓형). ⛔ 서버 이전·IP 변경을 첫 원인으로 안내하지 마세요 |
