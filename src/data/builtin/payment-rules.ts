...
/**
 * 내장 문서 get_doc('mcp/payment-rules').
 *
 * 서버 instructions 가 4.2만 자까지 불어 붙는 순간 컨텍스트를 크게 먹었다(2026-09-23 실측).
 * instructions 에는 절대 금지·금액 규칙·호출 순서만 남기고, 코드를 직접 쓸 때 필요한 세부를 이 문서로 옮겼다.
 * 문구는 옮기기 전 instructions.ts 와 같다 — 규칙을 고치면 generate_payment_code 생성물과 함께 고친다.
 *
 * @comment_by ehowlsla
 * @date: 2026-09-23
 */
export const PAYMENT_RULES_DOC: { meta: DocMeta; content: string } = {
    meta: {
        path: 'mcp/payment-rules',
        title: '결제 연동 상세 규칙 — open_type·서버승인·redirect 결과·웹훅·정기결제·구독·본인인증·용어집',
        category: 'payment',
        description: 'MCP instructions 에서 옮긴 PG 연동 상세 규칙. 기기별 open_type, 서버승인(분리승인) 기본, redirect 결과 페이지 파라미터와 event 분기, confirmPayment 리턴값 검증, 웹훅 IP·응답·webhook_type, 정기결제·구독 상태 관리·법적 고지, 본인인증 저장 원칙, 용어집, 영어→한국어 검색 키워드, 초보·Windows 설치 안내, 도구 사용 가이드.',
        keywords: [
            '결제 연동', '규칙', 'open_type', 'redirect', 'iframe', '서버승인', '분리승인', 'separately_confirmed',
            'confirmPayment', 'receiptPayment', 'cancelPayment', '웹훅', 'webhook', 'webhook_type', '223.130.82',
            '정기결제', '빌링키', 'requestSubscribePayment', '예약결제', '구독', '회차', '무료체험', '본인인증',
            'certificate', '용어집', 'Windows', '설치', '@latest', 'PG 코드',
        ],
    },
    content: `# 결제 연동 상세 규칙 — PG 핵심 규칙·용어집·설치 안내

이 문서는 MCP 서버 instructions 에서 옮겨 온 상세 규칙이다. instructions 에는 절대 금지·금액 규칙·호출 순서만 남기고,
코드를 직접 쓰거나 검토할 때 필요한 세부(open_type·승인 방식·redirect 파라미터·웹훅·정기결제·구독·본인인증)는 여기서 읽는다.
\`generate_payment_code\` 가 만든 코드는 이미 이 규칙을 따른다 — 손으로 고치거나 새로 쓸 때 이 문서를 기준으로 삼는다.
