...
## Commerce 호스팅 주문서 로그인 인계 — API URL 계약

호스팅 주문서에서 로그인이 풀리는 문제를 PG 설정/관리자 키 생성으로 해결하지 마세요. 상세 계약·재시도 표·검증 체크리스트: get_doc('commerce/hosted-checkout-session').

1. POST /v1/orders/prepare는 서버(BFF)만 호출합니다. 회원이면 Commerce JWT를 Bootpay-User-JWT 헤더로, 본문에 use_auto_login:true·response_type:"url"을 보냅니다(결제 위젯 user_token과 다름). 가맹점 HttpOnly 쿠키는 호스팅 도메인과 공유되지 않습니다.
2. 이때 API는 **/b/{client_key}?__s=…&redirect_url=…** 를 반환해야 합니다. 응답 url은 검사만 하고 **그대로 사용**하며 __s 암호화·/b 재조립 코드를 만들지 않습니다(__s는 bearer 세션, 로그 금지). true인데 직접 주문서 URL이면 API 계약 결함(AUTO_LOGIN_BRIDGE_MISSING)이므로 생성된 주문을 기록하고 자동 재주문하지 않습니다.
3. Invoice 전용 /i/{clientKey}/{invoiceId}/session에 일반 주문번호를 넣지 않습니다. 일반 주문의 세션 인계는 기존 /b입니다.
4. 재시도는 저장된 주문의 실제 상태를 먼저 조회합니다. order_expired+receipt_ready 확정 후 다음 구매자 클릭에서만 새 requestId를 만들고, pending/paid/unknown/timeout은 새 prepare 0건입니다. 3737·3770·session_not_found·session_mismatch를 한 문구로 뭉치지 않습니다.
5. 검증은 요청 캡처와 호스팅 세션 복원(/b 후 __tc, 세션 회원 = 주문 소유자)을 따로 확인합니다. URL 모양이나 모의 테스트만으로 운영 로그인 완료를 주장하지 않고, 진단 중 새 주문·결제 승인/취소를 실행하지 않습니다.

commerce_prepare_order는 user_jwt가 있으면 use_auto_login 기본 true, 없으면 false이고 명시적 false를 유지합니다. url 모드 true 응답에 /b가 없으면 AUTO_LOGIN_BRIDGE_MISSING입니다. 실제 주문을 만드는 도구이므로 진단만을 위해 반복 호출하지 마세요.
...
## 상품 상세 본문(content) 렌더 — 소비 계약

상품 상세 화면을 만들면 먼저 \`get_doc('product/block-rendering')\` 을 읽으세요(작성은 product/content-blocks, 렌더는 이 문서).
- 비어 있지 않은 compiled \`content\` 가 1순위입니다. content_type 이 image 여도 버리지 않고, 중간 이미지를 뽑아 갤러리로 옮기거나 content_images 를 덧붙이지 않습니다.
- content 가 비었거나 정제 후 남는 것이 없을 때만 content_images 를 원래 순서로, 둘 다 없으면 명시적 빈 상태. content_blocks 는 편집 원본이라 렌더에 쓰지 않습니다.
- 원문을 dangerouslySetInnerHTML/v-html 로 넣지 말고 허용 태그·속성만 다시 만듭니다(script·on*·javascript:·iframe·form 제거, https 또는 루트 상대 URL 만). 깨진 이미지는 그 자리에 대체 문구.
- 공유 CSS·클래스 계약은 generate_commerce_code(framework="nextjs") 생성물 vendor/product-detail/ 에 commit·SHA-256 manifest 와 함께 들어 있습니다. 공개 CDN/npm URL 을 지어내지 마세요.

---

## Commerce 기본 쇼핑몰 — 필수 UX와 완료 판정

회원가입·로그인·마이페이지·본인 주문·배송지·정보 수정·비밀번호 변경/복구·탈퇴는 기본 필수 기능입니다. get_doc("commerce/required-features")와 completion.required_features 순서대로 구현하세요. api_blocked는 미지원 API의 task로 연결하고 전체 쇼핑몰은 incomplete로 유지합니다. 관리자 users/:id나 내부 mall API로 대체하지 마세요.
...
상세 규칙: get_doc('commerce/storefront-baseline').
...
### "알림톡 보내줘" / "알림톡 템플릿 만들어줘" / "카카오 알림톡 연동" → alimtalk_* 도구 (stdio)
\`\`\`
alimtalk_status → [blockers 해결] → 템플릿 고르기 → alimtalk_send 미리보기 → 사용자 동의 → alimtalk_send(confirm_token)
\`\`\`
아래 "알림톡" 절을 따르세요.
...
알림톡 도구는 결제용 키가 아니라 **커머스 연동키**(Basic Auth)로 commerce-api \`/v1/alimtalk\` 를 부릅니다. stdio 에서만 노출됩니다.
...
**2. 템플릿은 공식 먼저, 없으면 자체**
- 보낼 문장이 있으면 \`alimtalk_recommend_official_templates(text)\` 부터. 공식 템플릿은 그룹키 채널이면 검수 없이 바로 발송됩니다
- 공식 카탈로그는 부트페이가 등록하는 대로 늘어납니다. **검색에 없는 템플릿 코드를 지어내지 마세요**
...
- 알림톡은 **정보성** 채널입니다. 예약 · 결제 · 가입처럼 고객 이용에 따른 안내로 지으세요. 할인 · 이벤트 · 신상품 홍보는 반려됩니다
...
- \`alimtalk_send\` / \`alimtalk_send_bulk\` 를 먼저 **confirm_token 없이** 불러 미리보기를 받습니다
- 미리보기의 message · recipients · warnings · must_tell_user 를 **사용자에게 그대로 보여 주고** "보내도 된다"는 답을 받은 뒤에만, 같은 인자에 confirm_token 을 더해 다시 부릅니다
- **사용자 동의 없이 confirm_token 을 넣지 마세요.** 샌드박스가 없어 곧바로 실제 카카오톡이 나가고 과금됩니다
- 인자를 하나라도 바꾸면 토큰이 무효입니다. 미리보기부터 다시 하세요
- 1회 발송 인원 상한(\`BOOTPAY_ALIMTALK_MAX_RECIPIENTS\`, 기본 100명)과 보낼 채널(\`BOOTPAY_ALIMTALK_SENDER_KEY\`)은 env 로 고정되어 도구 인자로 바꿀 수 없습니다. 상한을 넘는 대량 발송은 가맹점 서버 코드로 보내도록 안내하세요

**4. 하지 말 것**
- **문자(LMS) 대체발송을 안내하거나 제안하지 마세요.** 부트페이는 제공하지 않습니다 — 알림톡이 실패하면 실패로 끝나고 과금되지 않습니다
- 연동 채널이 둘 이상이면 보낼 채널을 사용자가 env \`BOOTPAY_ALIMTALK_SENDER_KEY\`(카카오 발신 프로필 키 — \`ksp_id\` 아님)로 고정해야 합니다. 이 키는 \`alimtalk_list_senders\` 에 나오지 않으니 지어내지 말고, 사용자가 모르면 부트페이에 문의하게 하세요
- 채널 해지(\`alimtalk_release_sender\`)는 이미 예약된 발송을 취소하지 않습니다. 해지 전에 미리보기의 reserved_pending 을 보여 주고 \`alimtalk_cancel_reserved\` 로 정리하세요
- 알림톡 웹훅 URL 을 주문 · 구독 웹훅 URL 과 같게 두지 마세요 — 기존 주문 연동이 모르는 payload 를 받아 깨집니다

**5. 가맹점 서버에 알림톡 발송 코드를 붙일 때**
- \`POST https://api.bootapi.com/v1/alimtalk/send\` (대량은 \`/send/bulk\`), 인증은 커머스 연동키 Basic Auth
- 템플릿의 \`required_variables\` 를 모두 채우고, 주문번호 · 예약번호 같은 고유값을 \`ref_id\` 로 넣어 재시도 중복 발송을 막으세요
- 예약 발송 \`reserved_at\` 은 시간대 오프셋을 붙인 ISO8601 로 보내세요 (해석 못 하는 값은 즉시 발송됩니다)
- 결과는 알림톡 웹훅(301 성공 · 302 실패)으로 받고 \`X-Bootpay-Signature\` HMAC 서명을 검증하세요
...
## Commerce 쇼핑몰 도구 범위 (stdio vs Remote)

- stdio 의 commerce_* 도구(BOOTPAY_COMMERCE_ENABLED=true)는 상점·상품·리뷰·회원 로그인/세션만 제공한다. 장바구니 검증·commerce_prepare_order·주문 상태/결과 조회는 Commerce Remote MCP 에만 있으니 stdio 세션에서 호출하라고 안내하지 않는다.
- 상품 정렬은 GET /v1/products 가 읽는 값(position, -created_at, created_at, price, -price, -sold)과 keyword 만 전달한다. latest/price_asc/price_desc/popular 는 변환되고 리뷰순 정렬은 서버에 없다.
- commerce_calculate_price 는 로컬 예상치다. 신규 주문은 POST /v1/cart/order-preview의 checkout_ready·quote_token·amounts를 사용하고 같은 구매자·기기로 prepare한다. 배송지 필요·amounts:null은 진행 불가이며 legacy summary로 대체하지 않는다.
- 구매자 결과는 GET /v1/orders/{order_number}/result: 회원 JWT 또는 비회원 result_token+같은 device UUID. 결과 토큰은 서버에 보관하고 payment_status·Retry-After를 따른다.
- 개발 전에 get_doc("commerce/storefront-development")로 현재 invoice HTML/CSS 규칙과 내부 mall API/public v1 경계를 확인한다. nextjs 외 생성기는 구현 가이드만 제공한다. 기존 프레임워크를 유지하며 명시적인 BFF를 구현한다.
- 도구 응답의 success/data 는 MCP 래퍼다. Node SDK 는 API 본문(예: 상품 목록 {list, count})을 그대로 반환한다.
