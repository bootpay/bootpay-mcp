...
## 0. 먼저: MCP 를 최신 버전으로 설치

```bash
node -v                                # Node.js 18 이상 필요. 없으면 https://nodejs.org 에서 LTS 설치
npm view @bootpay/mcp version          # npm 의 최신 버전
npx -y @bootpay/mcp@latest --version   # 지금 실행되는 버전 (v2.1.1 이상에서 지원)
```

두 버전이 다르면 옛 사본이 캐시에서 실행되고 있는 것입니다. 캐시를 비우고 다시 받으세요:

```bash
npx clear-npx-cache
```

> **MCP 설정에는 항상 `@latest` 를 붙이세요** — `npx -y @bootpay/mcp@latest`.
> 버전을 생략하면 npx 는 예전에 받아둔 사본을 계속 씁니다. 그 사이 고쳐진 문서 검색·SDK 버전표가 반영되지 않습니다.

---

## 방법 A: Admin CLI (MCP stdio) — 권장

MCP를 stdio 모드(`npx -y @bootpay/mcp@latest`)로 연결하면 AI가 프로젝트 생성부터 키 발급까지 자동으로 처리합니다.

> **⚠️ HTTP 모드에서는 Admin 도구를 사용할 수 없습니다.** 반드시 stdio(`npx -y @bootpay/mcp@latest`)로 연결하세요.
...
```
→ `client_key` 조회 가능 (secret_key는 마스킹됨)
...
| source | 조회 대상 | 용도 |
|--------|----------|------|
| `core` | 결제용 키체인 (AppKeyChain) | 결제 연동 — Client Key, Secret Key |
...
|---------------|-------------|------------|------|
| **Client Key** (= REST API Key) | `client_key` | `BOOTPAY_CLIENT_KEY` | 프론트엔드 SDK 결제창 호출 + 서버 Basic Auth 공통 |
...
| 잘못된 이름 | 올바른 이름 | 비고 |
|------------|------------|------|
| `BOOTPAY_REST_APP_ID` | `BOOTPAY_CLIENT_KEY` | "REST API Key"는 Admin UI 라벨이지 변수명이 아님 |
...
> Sandbox와 Production의 Client Key는 다릅니다. `get_auth_status`로 현재 환경을 확인하세요.
...
MCP를 사용할 수 없는 경우 **사용자가 직접** [Bootpay 관리자](https://admin.bootpay.co.kr)에서 키를 확인해야 합니다.

> **🚫 AI가 API 키를 추측하거나 임의로 생성하는 것은 절대 금지입니다.**
> Bootpay API 키는 Bootpay 서버가 발급한 값만 유효합니다.
> 사용자가 실제 키를 제공하기 전까지 `.env` 작성이나 코드 생성을 진행하지 마세요.

1. [회원가입](https://admin.bootpay.co.kr) → 사업자 정보 등록
2. [프로젝트 생성](https://admin.bootpay.co.kr/project/new)
3. [PG사 활성화](https://admin.bootpay.co.kr/payment/setting)
4. [API 연동키 확인](https://admin.bootpay.co.kr/setting/developer?tab=api-key&cursor=payment)

**사용자에게 이렇게 안내하세요:**

```
admin.bootpay.co.kr → 개발자 설정 → API 연동키(결제) 탭에서 아래 2개를 복사해서 알려주세요:

1. Client Key (= REST API Key)
...
```bash
BOOTPAY_CLIENT_KEY=6b...      # 1번: Client Key (프론트엔드 SDK + 서버 Basic Auth 공통)
BOOTPAY_SECRET_KEY=dG9r...    # 2번: Secret Key (서버 전용)
```

> **중요**: Admin UI에서 "REST API Key"라고 표시되는 값이 `BOOTPAY_CLIENT_KEY`이고, "Private Key"라고 표시되는 값이 `BOOTPAY_SECRET_KEY`입니다. 이름이 다르지만 같은 값입니다.
