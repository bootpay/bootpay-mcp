...
빌링키·정기결제 상태를 관리하는 테이블 스키마는 `billing.md`의 [DB 설계](./billing.md#db-설계) 섹션(`billing_keys` + `recurring_payments`)을 참고하세요.

## 1. 빌링키로 자동결제

프론트엔드에서 받은 `receipt_id`로 `lookupSubscribeBillingKey`를 호출해 확보한 `billing_key`를 사용합니다.
...
> ⚠️ **해지 전 대기 예약부터 취소하세요.** `destroyBillingKey()`를 호출해도 등록된 예약결제는 자동으로 취소되지 않습니다. `cancelSubscribeReserve()`로 대기 중인 예약을 모두 취소한 뒤 해지해야 합니다.

```javascript
// ── 빌링키 해지 (대기 예약 취소 → 해지 순서) ──
async function destroyBillingKey(billingKey, pendingReserveIds = []) {
...
5. **빌링키 해지**: 구독 해지 시 대기 중인 예약을 `cancelSubscribeReserve()`로 먼저 취소한 뒤 `destroyBillingKey()` 호출하여 무효화 (예약은 자동 취소되지 않음)
