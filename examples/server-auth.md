...
async function getCertificate(receiptId) {
    try {
        ...
        // ⚠️ 핵심: status가 12(본인인증완료)일 때만 완료로 처리한다. 그 외 값은 미완료 상태다.
        if (result.status !== 12) {
        ...
        console.error('본인인증 조회 에러:', { receiptId, error: error.message || error })
        throw error
    }
}
```
...
async function requestAuth(userInfo) {
    try {
        ...
        console.error('본인인증 요청 에러:', { error: error.message || error })
        throw error
    }
}
...
async function confirmAuth(receiptId, otp) {
    try {
        ...
        console.error('OTP 확인 에러:', { receiptId, error: error.message || error })
        throw error
    }
}
...
async function resendSms(receiptId) {
    try {
        ...
        console.error('SMS 재전송 에러:', { receiptId, error: error.message || error })
        throw error
    }
}
```
...
async function publishCashReceipt(receiptId, identityNo, type) {
    try {
        ...
        console.error('현금영수증 발행 에러:', { receiptId, error: error.message || error })
        throw error
    }
}
...
async function cancelCashReceiptOnReceipt(receiptId) {
    try {
        ...
        console.error('현금영수증 취소 에러:', { receiptId, error: error.message || error })
        throw error
    }
}
```
...
async function requestStandaloneCashReceipt() {
    try {
        ...
        console.error('별건 현금영수증 발행 에러:', { error: error.message || error })
        throw error
    }
}
...
async function cancelStandaloneCashReceipt(receiptId) {
    try {
        ...
        console.error('별건 현금영수증 취소 에러:', { receiptId, error: error.message || error })
        throw error
    }
}
```
...
async function startShipping(receiptId, trackingInfo) {
    try {
        ...
        console.error('배송 등록 에러:', { receiptId, error: error.message || error })
        throw error
    }
}
...
async function getUserToken(userId) {
    try {
        ...
        console.error('사용자 토큰 발급 에러:', { userId, error: error.message || error })
        throw error
    }
}
```
