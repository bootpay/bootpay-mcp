...
1. **Client Key**: `.env`의 `VITE_BOOTPAY_CLIENT_KEY`에 설정. MCP stdio에서 `create_keychain(targets=["core"])` 또는 `list_keychains(source=core)`로 조회. AI가 임의 생성 금지
...
5. **통합결제창**: 이 예제는 `pg: 'nicepay'`, `method`를 지정한 단일 결제창 방식입니다. `pg`와 `method`를 생략하면 관리자에서 활성화한 모든 결제수단이 표시되는 **통합결제창**이 뜹니다 → `unified-payment.md` 참조
