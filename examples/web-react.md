...
```bash
# .env
VITE_BOOTPAY_APP_ID=your_application_id  # 관리자에서 발급받은 실제 ID로 교체하세요
```
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await Bootpay.requestPayment({
        ...
        application_id: import.meta.env.VITE_BOOTPAY_APP_ID,
        price: totalPrice,
...
1. **Application ID**: `import.meta.env.VITE_BOOTPAY_APP_ID` — `.env` 파일에 설정 필수
