...
<body>
    ...
    BootpayWidget.render('#bootpay-widget', {
        ...
        client_key: process.env.BOOTPAY_CLIENT_KEY, // .env에서 설정
        price: 50000,
        ...
        extra: {
            ...
            separately_confirmed: true  // 서버승인(분리승인) 활성화 → confirm 훅에서 서버가 승인
        },
        items: [
        ...
        hooks: {
            ...
            // 서버승인(기본): 승인 직전 호출 — receipt_id를 서버로 전달만 하고 승인은 서버가 수행
            confirm: async function(data) {
                // receipt_id를 서버로 전달만 — 승인은 서버가 confirmPayment()로 수행
                await fetch('/api/confirm', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ receipt_id: data.receipt_id, order_id: data.order_id }),
                })
                location.href = '/payment/result?order_id=' + (data.order_id || '')
            },

            // 결제 완료 — 서버승인 방식에서는 호출되지 않음
            done: function(data) {
                // 서버승인 방식에서는 호출되지 않음
                // 결과는 결과 화면에서 서버 DB 조회(polling)로 표시
            },
...
9. → confirm 이벤트: receipt_id를 서버로 전달 → 서버가 confirmPayment()로 승인
10. → 결과 화면에서 서버 DB 조회로 결과 확인 (done은 호출되지 않음, error/cancel은 그대로 수신)
```
...
7. **서버승인이 기본**: `extra.separately_confirmed: true` + `hooks.confirm`에서 receipt_id를 서버로 전달만. 서버가 `confirmPayment()` 후 리턴값(status·price)으로 검증. `hooks.done`은 호출되지 않음
8. **웹훅 보완**: 브라우저 이탈로 결과 처리가 유실될 수 있음. 웹훅 엔드포인트 필수 (부트페이 IP `223.130.82.0/24` 필터 + 200/`success:true` 응답 + 재검증)
