...
> AI가 임의로 생성한 값은 100% 실패합니다. Flutter는 Web/Android/iOS 별로 Application ID가 다릅니다.
...
BOOTPAY_WEB_APP_ID=         # Web Application ID
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  String get _appId {
    if (kIsWeb) return const String.fromEnvironment('BOOTPAY_WEB_APP_ID');
    if (Platform.isAndroid) return const String.fromEnvironment('BOOTPAY_ANDROID_APP_ID');
    if (Platform.isIOS) return const String.fromEnvironment('BOOTPAY_IOS_APP_ID');
    return '';
  }
