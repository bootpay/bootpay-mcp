...
> **⚠️ Client Key는 Admin CLI(`create_keychain` 또는 `list_keychains`)로 조회하거나, [admin.bootpay.co.kr](https://admin.bootpay.co.kr/setting/developer?tab=api-key&cursor=payment)에서 직접 확인한 실제 값을 사용하세요.**
> AI가 임의로 생성한 값은 100% 실패합니다. iOS 전용 Client Key가 별도로 발급됩니다.
...
class PaymentViewController: UIViewController {
    ...
    func requestPayment() {
        ...
        let payload = Payload()
        // ⚠️ Admin CLI list_keychains(source=core) 또는 admin.bootpay.co.kr에서 조회한 실제 iOS Client Key
        // AI가 이 값을 추측하거나 임의 생성하면 100% 실패합니다
        payload.clientKey = ProcessInfo.processInfo.environment["BOOTPAY_IOS_CLIENT_KEY"] ?? ""
        payload.pg = "nicepay"
        payload.method = "card"
        payload.price = 50000
        ...
        .onConfirm { data in
            ...
            self.sendConfirmToServer(data)  // 서버 /api/confirm 호출 → 서버가 confirmPayment()
            return false
        }
        .onDone { data in
            // 서버승인 방식에서는 호출되지 않음
    ...
    func sendConfirmToServer(_ data: String) {
        // let json = try? JSONSerialization.jsonObject(with: Data(data.utf8)) as? [String: Any]
        // URLSession 으로 서버 /api/confirm 에 POST → 서버가 confirmPayment 후 금액 검증
    }
}
```
...
class SubscriptionViewController: UIViewController {
    ...
    func requestSubscription() {
        ...
        let payload = Payload()
        // ⚠️ Admin CLI로 조회한 실제 iOS Client Key — AI 임의 생성 금지
        payload.clientKey = ProcessInfo.processInfo.environment["BOOTPAY_IOS_CLIENT_KEY"] ?? ""
        payload.pg = "nicepay"
...
1. **Client Key**: iOS 전용 키를 Admin CLI `list_keychains(source=core)`로 조회하거나 [관리자 → 개발자 설정](https://admin.bootpay.co.kr/setting/developer?tab=api-key&cursor=payment)에서 확인. AI가 임의 생성 금지
