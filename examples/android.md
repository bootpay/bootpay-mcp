...
// app/build.gradle
dependencies {
    implementation 'io.github.bootpay:android:5.2.0'
}
```
...
class PaymentActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val payload = Payload().apply {
            // ⚠️ Admin CLI list_keychains(source=core) 또는 admin.bootpay.co.kr에서 조회한 실제 Android Client Key
...
1. **Client Key**: Android 전용 키를 Admin CLI `list_keychains(source=core)`로 조회하거나 [관리자 → 개발자 설정](https://admin.bootpay.co.kr/setting/developer?tab=api-key&cursor=payment)에서 확인. `BuildConfig` 또는 `local.properties`로 주입 권장. AI가 임의 생성 금지
